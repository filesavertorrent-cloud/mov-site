...
import { fetchCounters, detectFileSize, getItemCounters } from '../services/counterService';
...
function Admin() {
    ...
    // Counters and file size detection
    const [counters, setCounters] = useState({ movies: {}, series: {} });
    const [detectingSize, setDetectingSize] = useState({});
    ...
    const loadConfig = async () => {
        ...
        try {
            ...
            // Fetch counters concurrently but non-blocking (already cached locally preferably)
            fetchCounters().then(c => setCounters(c)).catch(e => console.warn("Failed to load counters:", e));
    ...
    const handleUrlBlur = async (type, i, j, url) => {
        if (!url || url === '#' || url.trim() === '') return;
        
        const key = `${type}-${i}-${j}`;
        setDetectingSize(p => ({ ...p, [key]: true }));
        try {
            const size = await detectFileSize(url);
            if (size) {
                if (type === 'movie') {
                    handleQualityChange(i, j, 'size', size);
                } else if (type === 'series') {
                    handleEpisodeChange(i, j, 'size', size);
                }
            }
        } finally {
            setDetectingSize(p => ({ ...p, [key]: false }));
        }
    };
    ...
    const handleEpisodeChange = (seriesIdx, epIdx, field, value) => {
        const newSeries = [...config.series];
        const newEpisodes = [...newSeries[seriesIdx].episodes];
        newEpisodes[epIdx] = { ...newEpisodes[epIdx], [field]: value };
        newSeries[seriesIdx] = { ...newSeries[seriesIdx], episodes: newEpisodes };
        setConfig({ ...config, series: newSeries });
    };
    ...
    const addNewSeries = () => {
        ...
        setConfig({ ...config, series: [newSeries, ...(config.series || [])] });
    };

    const deleteSeries = (index) => {
        if (!window.confirm(`Delete series "${config.series[index].title}"?`)) return;
        const newSeries = config.series.filter((_, i) => i !== index);
        setConfig({ ...config, series: newSeries });
    };

    const addEpisode = (seriesIdx) => {
        const newSeries = [...config.series];
        const epCount = newSeries[seriesIdx].episodes.length;
        newSeries[seriesIdx] = {
            ...newSeries[seriesIdx],
            episodes: [...newSeries[seriesIdx].episodes, { label: `Episode ${epCount + 1}`, quality: "1080p", size: "0 GB", url: "" }]
        };
        setConfig({ ...config, series: newSeries });
    };

    const deleteEpisode = (seriesIdx, epIdx) => {
        const newSeries = [...config.series];
        newSeries[seriesIdx] = {
            ...newSeries[seriesIdx],
            episodes: newSeries[seriesIdx].episodes.filter((_, i) => i !== epIdx)
        };
        setConfig({ ...config, series: newSeries });
    };

    const handleSeriesImageUpload = async (index, file) => {
        if (!file) return;
        if (file.size > 2 * 1024 * 1024) {
            alert("File too large (Max 2MB)");
            return;
        }
        setStatus(`Uploading poster for series #${index + 1}...`, "loading");
        try {
            const url = await uploadImageToRepo(pat, GITHUB_OWNER, GITHUB_REPO, file);
            handleSeriesChange(index, 'poster', url);
            setStatus("Poster uploaded successfully! ✅", "success");
        } catch (err) {
            setStatus("Poster upload failed: " + err.message, "error");
        }
    };
    ...
    const saveAndPush = async () => {
        ...
        try {
            ...
            const configToSave = { ...config, movies: updatedMovies, series: config.series || [] };
            ...
            <div className="admin-container">
                ...
                <div className="admin-stats fade-in">
                    ...
                    <div className="stat-chip">
                        ...
                        <span className="stat-chip__value">{(config.series || []).length}</span>
                    </div>
                    <div className="stat-chip">
                ...
                <div className="admin-section fade-in">
                    ...
                    <div className="movies-list">
                        {config.movies.map((movie, i) => {
                            const movieStats = getItemCounters(counters, 'movies', movie.title);
                            const totalDownloads = Object.values(movieStats.downloads || {}).reduce((a, b) => a + b, 0);
                            return (
                                    ...
                                    {movie.qualities.map((q, qi) => (
                                        <div key={qi} className="quality-row">
                                            ...
                                            <input
                                                ...
                                                placeholder="Size"
                                                className={`quality-size-input ${detectingSize[`movie-${i}-${qi}`] ? 'size-detecting' : ''}`}
                                            />
                                            <input
                                                type="url"
                                                value={q.url === '#' ? '' : q.url}
                                                onChange={e => handleQualityChange(i, qi, 'url', e.target.value || '#')}
                                                onBlur={e => handleUrlBlur('movie', i, qi, e.target.value)}
                                                placeholder={`Paste ${q.label} link...`}
                                                className="quality-url-input"
                                            />
                                        </div>
                                    ))}
                                    {(movieStats.likes > 0 || totalDownloads > 0) && (
                                        <div className="admin-item-stats">
                                            {movieStats.likes > 0 && <span className="admin-stat-badge admin-stat-badge--likes">❤️ {movieStats.likes} Likes</span>}
                                            {totalDownloads > 0 && <span className="admin-stat-badge admin-stat-badge--downloads">⬇️ {totalDownloads} Downloads</span>}
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                {/* Series Section */}
                <div className="admin-section fade-in">
                    <div className="admin-section__title">📺 Web Series ({(config.series || []).length})</div>
                    <button className="btn-add-series" onClick={addNewSeries}>
                        ➕ Add New Series
                    </button>

                    <div className="series-list">
                        {(config.series || []).map((series, i) => {
                            const seriesStats = getItemCounters(counters, 'series', series.title);
                            const totalDownloads = Object.values(seriesStats.downloads || {}).reduce((a, b) => a + b, 0);
                            return (
                                <div key={i} className="series-edit-card fade-in" style={{ animationDelay: `${i * 0.03}s` }}>
                                    <div className="series-edit-card__header">
                                    <div className="movie-edit-card__poster-wrap">
                                        <img
                                            src={series.poster}
                                            alt={series.title}
                                            className="movie-edit-card__poster"
                                            onError={e => { e.target.src = 'https://via.placeholder.com/80x120/1a1a2e/a78bfa?text=No+Image'; }}
                                        />
                                        <button
                                            className="poster-upload-btn"
                                            onClick={() => {
                                                ...
                                                input.type = 'file';
                                                input.accept = 'image/*';
                                                input.onchange = (e) => handleSeriesImageUpload(i, e.target.files[0]);
                                                input.click();
                                            }}
                                            title="Upload poster"
                                        >
                                    ...
                                    <div className="series-edit-card__fields">
                                        <div className="field-row">
                                            <label>Series Title</label>
                                            <input
                                                type="text"
                                                value={series.title}
                                                onChange={e => handleSeriesChange(i, 'title', e.target.value)}
                                        ...
                                        <div className="field-row">
                                            ...
                                            <input
                                                type="url"
                                                value={series.poster}
                                                onChange={e => handleSeriesChange(i, 'poster', e.target.value)}
                                                placeholder="https://..."
                                            />
                                        </div>
                                        <div className="field-grid-2">
                                            <div className="field-row">
                                                ...
                                                <input
                                                    type="text"
                                                    value={series.genre}
                                                    onChange={e => handleSeriesChange(i, 'genre', e.target.value)}
                                            ...
                                            <div className="field-row">
                                                ...
                                                <input
                                                    type="text"
                                                    value={series.year}
                                                    onChange={e => handleSeriesChange(i, 'year', e.target.value)}
                                                    placeholder="2026"
                                                />
                                            </div>
                                        </div>
                                        <div className="series-edit-card__actions">
                                            <button className="btn-delete-series" onClick={() => deleteSeries(i)}>
                                                🗑️ Delete Series
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div className="episodes-section">
                                    <div className="episodes-section__label">
                                        <span>Episodes ({series.episodes.length})</span>
                                        <button className="btn-add-episode" onClick={() => addEpisode(i)}>
                                            ➕ Add Episode
                                        </button>
                                    </div>
                                    {series.episodes.map((ep, ei) => (
                                        <div key={ei} className="episode-edit-row">
                                            <div className="episode-edit-row__label-input">
                                                <input
                                                    type="text"
                                                    value={ep.label}
                                                    onChange={e => handleEpisodeChange(i, ei, 'label', e.target.value)}
                                                    placeholder="Episode 1"
                                                />
                                            </div>
                                            <div className="episode-edit-row__quality-input">
                                                <input
                                                    type="text"
                                                    value={ep.quality}
                                                    onChange={e => handleEpisodeChange(i, ei, 'quality', e.target.value)}
                                                    placeholder="1080p"
                                                />
                                            </div>
                                            <div className="episode-edit-row__size-input">
                                                <input
                                                    type="text"
                                                    value={ep.size}
                                                    onChange={e => handleEpisodeChange(i, ei, 'size', e.target.value)}
                                                    placeholder="Size"
                                                    className={detectingSize[`series-${i}-${ei}`] ? 'size-detecting' : ''}
                                                />
                                            </div>
                                            <div className="episode-edit-row__url-input">
                                                <input
                                                    type="url"
                                                    value={ep.url === '#' ? '' : ep.url}
                                                    onChange={e => handleEpisodeChange(i, ei, 'url', e.target.value || '#')}
                                                    onBlur={e => handleUrlBlur('series', i, ei, e.target.value)}
                                                    placeholder="Paste episode link..."
                                                />
                                            </div>
                                            <button className="btn-delete-episode" onClick={() => deleteEpisode(i, ei)} title="Remove episode">
                                                ✕
                                            </button>
                                        </div>
                                    ))}
                                    {series.episodes.length === 0 && (
                                        <div className="request-empty">No episodes yet — click "Add Episode" above</div>
                                    )}
                                    {(seriesStats.likes > 0 || totalDownloads > 0) && (
                                        <div className="admin-item-stats">
                                            {seriesStats.likes > 0 && <span className="admin-stat-badge admin-stat-badge--likes">❤️ {seriesStats.likes} Likes</span>}
                                            {totalDownloads > 0 && <span className="admin-stat-badge admin-stat-badge--downloads">⬇️ {totalDownloads} Downloads</span>}
                                        </div>
                                    )}
                                </div>
